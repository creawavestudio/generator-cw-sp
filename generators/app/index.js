...
class CreaWSP extends Generator {
  ...
  async prompting() {
    ...
    const prompts = [{
      ...
      },
      {
        ...
        default: shelljs.exec("git config user.name", {
          silent: true
        }).replace(/\n/g, "") || ""
      },
      {
        ...
        default: shelljs.exec("git config user.email", {
          silent: true
        }).replace(/\n/g, "") || ""
      },
      {
        ...
        default: shelljs.exec("git config github.user", {
          silent: true
        }).replace(/\n/g, "") || ""
      },
      {
        ...
        choices: ["Apache 2.0", "MIT", "Mozilla Public License 2.0", "BSD 2-Clause (FreeBSD) License", "BSD 3-Clause (NewBSD) License", "Internet Systems Consortium (ISC) License", "GNU AGPL 3.0", "GNU GPL 3.0", "GNU LGPL 3.0"]
      }
    ];

    while (true) {
      let props = await this.prompt(prompts);
      let opts = await initOptsJSON(props);
  ...
  async writing() {
    ...
    const tasks = [
      ...
      "\"./gulp/tasks/other/uncss\"", "\"./gulp/tasks/other/sprite\""
    ];
    ...
    this.fs.copy(
      ...
      this.destinationPath("gulp/config/config.json"), {
        process: content => {
          ...
          new_content.cfg.srv.port1 = isNaN(+this.proc.PORT) ? +this.proc.PORT : 3000;
          new_content.cfg.srv.port2 = isNaN(+this.proc.PORT) ? 1 + +this.proc.PORT : 3001;
...
async function initOptsJSON(props) {
  console.log(props)
